...
bool CoreChecks::CheckDependencyExists(const uint32_t subpass, const VkImageLayout layout,
    ...
    for (uint32_t k = 0; k < dependent_subpasses.size(); ++k) {
        ...
        if (static_cast<uint32_t>(subpass) == sp.index) continue;
...
bool CoreChecks::ValidateDependencies(FRAMEBUFFER_STATE const *framebuffer, RENDER_PASS_STATE const *renderPass) {
    ...
    for (uint32_t i = 0; i < pCreateInfo->attachmentCount; ++i) {
        for (uint32_t j = i + 1; j < pCreateInfo->attachmentCount; ++j) {
            ...
            if (viewi == viewj) {
                attachments[i].overlapping.push_back(j);
                attachments[j].overlapping.push_back(i);
                continue;
            }
    ...
    for (uint32_t i = 0; i < pCreateInfo->subpassCount; ++i) {
        ...
        for (uint32_t j = 0; j < subpass.inputAttachmentCount; ++j) {
            ...
            attachments[attachment].inputs.push_back(sp);
            for (auto overlapping_attachment : attachments[attachment].overlapping) {
                attachments[overlapping_attachment].inputs.push_back(sp);
            }
        }
        for (uint32_t j = 0; j < subpass.colorAttachmentCount; ++j) {
            ...
            attachments[attachment].outputs.push_back(sp);
            for (auto overlapping_attachment : attachments[attachment].overlapping) {
                attachments[overlapping_attachment].outputs.push_back(sp);
            }
