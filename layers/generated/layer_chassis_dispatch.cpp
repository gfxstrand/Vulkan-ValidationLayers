    ...
    VkFence                                     fence)
{
    ...
    if (local_pSubmits) {
        for (uint32_t index0 = 0; index0 < submitCount; ++index0) {
            FreePnextChain(const_cast<void *>(local_pSubmits[index0].pNext));
        }
    ...
    VkDeviceMemory*                             pMemory)
{
    ...
    if (local_pAllocateInfo) {
        FreePnextChain(const_cast<void *>(local_pAllocateInfo->pNext));
        delete local_pAllocateInfo;
    }
    if (VK_SUCCESS == result) {
        std::lock_guard<std::mutex> lock(dispatch_lock);
    ...
    VkImage*                                    pImage)
{
    ...
    if (local_pCreateInfo) {
        FreePnextChain(const_cast<void *>(local_pCreateInfo->pNext));
        delete local_pCreateInfo;
    }
    if (VK_SUCCESS == result) {
        std::lock_guard<std::mutex> lock(dispatch_lock);
    ...
    const VkCopyDescriptorSet*                  pDescriptorCopies)
{
    ...
    if (local_pDescriptorWrites) {
        for (uint32_t index0 = 0; index0 < descriptorWriteCount; ++index0) {
            FreePnextChain(const_cast<void *>(local_pDescriptorWrites[index0].pNext));
        }
    ...
    VkSubpassContents                           contents)
{
    ...
    if (local_pRenderPassBegin) {
        FreePnextChain(const_cast<void *>(local_pRenderPassBegin->pNext));
    ...
    const VkBindImageMemoryInfo*                pBindInfos)
{
    ...
    if (local_pBindInfos) {
        for (uint32_t index0 = 0; index0 < bindInfoCount; ++index0) {
            FreePnextChain(const_cast<void *>(local_pBindInfos[index0].pNext));
        }
        delete[] local_pBindInfos;
    }
    return result;
    ...
    VkImageFormatProperties2*                   pImageFormatProperties)
{
    ...
    if (local_pImageFormatInfo) {
        FreePnextChain(const_cast<void *>(local_pImageFormatInfo->pNext));
